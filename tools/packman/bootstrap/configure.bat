...
set PM_PYTHON_VERSION=3.12.13-nv1-windows-x86_64
...
set PM_PYTHON_PACKAGE=python@%PM_PYTHON_VERSION%.tar.gz
for /f "delims=" %%a in ('powershell -ExecutionPolicy ByPass -NoLogo -NoProfile -File "%~dp0\generate_temp_file_name.ps1"') do set TEMP_FILE_NAME=%%a
set TARGET=%TEMP_FILE_NAME%.tar.gz
...
:: Extract to %TEMP% first to avoid "Access is denied" when PM_PACKAGES_ROOT is on
:: network drives, OneDrive, or other restricted paths. Then move to final location.
for /f "delims=" %%a in ('powershell -ExecutionPolicy ByPass -NoLogo -NoProfile -File "%~dp0\generate_temp_folder.ps1" -parentPath "%TEMP%"') do set TEMP_FOLDER_NAME=%%a
echo Unpacking Python interpreter ...
"%SystemRoot%\system32\tar.exe" -xzf "%TARGET%" -C "%TEMP_FOLDER_NAME%"
del "%TARGET%"
...
move /y "%TEMP_FOLDER_NAME%" "%PM_PYTHON_DIR%" > nul 2>&1
...
:PACKMAN
:: Enable these lines for debugging DLLs shipped with Python executable
:: echo Python %PM_PYTHON_VERSION% at %PM_PYTHON_DIR%
:: echo --- Bundled DLLs ---
:: dir /b "%PM_PYTHON_DIR%\*.dll" 2>nul || echo   (none)
:: echo ---
...
:: Pad [xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx]
:: This is to prevent the label from landing on the boundary of a 512 byte block. 
