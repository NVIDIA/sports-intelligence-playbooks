...
# Slurm batch script: containerized multi-node AutoModel inference.
...
srun --nodes="${SLURM_NNODES}" \
  ...
  bash -c '
    ...
    # High-resolution video prefill is one large allocation; avoids fragmentation OOM.
    export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-expandable_segments:True}"

    if [[ -f "${REPO_ROOT}/avlm/utils/_source_params.sh" ]]; then
      # shellcheck source=/dev/null
      source "${REPO_ROOT}/avlm/utils/_source_params.sh"
      # Use the container nemo_automodel source + /opt/venv env (same as training).
    ...
    torchrun \
      ...
      "${REPO_ROOT}/avlm/inference/common/run_inference.py" \
      "${infer_args[@]}"
  ' &> "${LOG_FILE}"
