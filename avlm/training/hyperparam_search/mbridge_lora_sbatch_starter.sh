#!/usr/bin/env bash
# Submit one Slurm Megatron-Bridge LoRA job per trial in a hyperparameter-search YAML.
#
...
hyperparam_mode=mbridge_lora
avlm_utils_dir="${script_dir}/../../utils"
slurm_dir="$(cd -- "${script_dir}/../megatron-bridge/lora/slurm" && pwd)"
...
_hyperparam_build_submit_args() {
  ...
  for key in "${SBATCH_CLI_OVERRIDE_KEYS[@]}"; do
    case "${key}" in
      MODEL_NAME|CONFIG_YAML_REL|OUTPUT_BASE|LOGS_DIR) continue ;;
    esac
    if [[ -n "${!key+x}" ]]; then
      _out+=("${key}=${!key}")
    fi
