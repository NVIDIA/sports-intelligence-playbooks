...
[[ -n "${_TRAIN_LIB_LOADED:-}" && $(type -t sft_train 2>/dev/null) == function ]] && return 0
_TRAIN_LIB_LOADED=1

_SFT_LIB_DIR="$(cd -- "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
sft_load_recipe_env() {
  ...
  if [[ $(type -t apply_yaml_exports_preserving_cli 2>/dev/null) != function ]]; then
    ...
    source "${_SFT_LIB_DIR}/../../../../utils/_source_params.sh"
  fi
  _py=/opt/venv/bin/python3
  [[ -x "${_py}" ]] || _py=$(command -v python3)
  _yaml_params="$("${_py}" "${_SFT_LIB_DIR}/../../../../utils/_load_params_yaml.py" "${recipe_yaml}" \
...
# Validate parallelism against a planned or active GPU layout (sbatch preflight or train job).
sft_validate_parallelism_layout() {
...
sft_export_cpu_memory_env() {
  # Pretokenize-on-the-fly (video decode + ffmpeg) × many ranks can exhaust host RAM.
  # MALLOC_ARENA_MAX limits glibc per-thread arenas; thread caps avoid BLAS oversubscription.
...
sft_resolve_output_dirs() {
  local _nodes="${SLURM_NNODES:-${num_nodes:-1}}"
  local _gpus="${NPROC_PER_NODE:-${GPUS_PER_NODE:?GPUS_PER_NODE must be set}}"
  sft_validate_parallelism_layout "${_nodes}" "${_gpus}"
  ...
  export OUTPUT_BASE="${OUTPUT_BASE:-${_SFT_LIB_DIR}/outputs}"
...
sft_launch_automodel() {
  ...
  if [[ "${SLURM_NNODES:-1}" -gt 1 ]]; then
    ...
    _rdzv_id="${SLURM_JOB_ID:-${RDZV_ID:-sft_train}}"
    _torchrun_args+=(
      --nnodes="${SLURM_NNODES}"
      --rdzv_backend=c10d
  ...
  # setsid + trap: Ctrl+C kills the full torchrun tree (needed during NCCL/checkpoint).
...
sft_train() {
  ...
  _TRAIN_SCRIPT_DIR="${_SFT_LIB_DIR}"
  # shellcheck source=../../../../utils/_train_env.sh
  source "${_SFT_LIB_DIR}/../../../../utils/_train_env.sh"
  ...
  if [[ -n "${VAL_EVERY_STEPS:-}" ]]; then
    _extra_args+=(--step_scheduler.val_every_steps="${VAL_EVERY_STEPS}")
  fi
  ...
  if [[ -n "${CHECKPOINT_DIR:-}" ]]; then
    # Avoid accidental resume from stale/default YAML checkpoint paths when DP world size changes.
  ...
  if [[ "${USE_SEQUENCE_PACKING}" == "1" && -n "${PACK_SIZE:-}" ]]; then
