...
# Submit AutoModel SFT through Slurm. See avlm/training/automodel/sft/SFT_GUIDE.MD.
set -euo pipefail
...
# Training defaults from params YAML.
...
if [[ "${SLURM_ACCOUNT_RACE}" == "1" || "${SLURM_ACCOUNT_RACE}" == "true" ]]; then
  ...
  # CLI override when not racing: SLURM_ACCOUNTS="acc1 acc2" bash sbatch_starter.sh
...
sft_load_recipe_env "${CONFIG_YAML}"
sft_validate_parallelism_layout "${num_nodes}" "${GPUS_PER_NODE}"

# Host-RAM / recovery: pretokenize-on-the-fly is CPU-heavy; gc + frequent ckpts limit loss on OOM.
export GC_EVERY_STEPS="${GC_EVERY_STEPS:-1}"
sft_export_cpu_memory_env
...
export TRAIN_LAUNCH_KIND="${TRAIN_LAUNCH_KIND:-batch}"

export OUTPUT_BASE="${OUTPUT_BASE:-${slurm_dir}/outputs}"
export OUTPUT="${OUTPUT:-${OUTPUT_BASE}/${MODEL_NAME}}"
# Include topology in checkpoint dir by default so multi-node/single-node runs
# do not resume incompatible dataloader states from a shared path.
...
# WandB run name is passed via env (WANDB_NAME); srun.sh builds CLI overrides in _train_lib.sh.
...
DRY_RUN=1 — resolved sbatch config (no directories created, no submit)
  ...
  CHECKPOINT_DIR    ${CHECKPOINT_DIR:-${OUTPUT}/checkpoints_${CKPT_LAYOUT_TAG}}
...
_sft_print_detach_notice() {
  ...
  rel_logs="$(_sft_relpath "${LOGS_DIR}")"
  rel_submit="$(_sft_relpath "${submit_log}")"
  if [[ -n "${MAX_STEPS:-}" ]]; then
    steps_suffix="  |  steps ${MAX_STEPS}"
  fi

  if [[ "${USE_SEQUENCE_PACKING}" == "1" ]]; then
    _sft_data_mode="pack ${PACK_SIZE}"
  else
    _sft_data_mode="collate ${COLLATE_MAX_LENGTH} / ${MAX_VIDEO_FRAMES}fr"
  fi

  cat <<EOF

════════════════════════════════════════════════════════════
 SFT batch submit (detached — account race in background)
════════════════════════════════════════════════════════════
 Model       ${MODEL_NAME}
 Cluster     ${num_nodes} nodes × ${GPUS_PER_NODE} GPUs  |  GBS ${GLOBAL_BATCH_SIZE}${steps_suffix}
 Parallel    TP=${_tp}  PP=${_pp}  CP=${_cp}  EP=${EP}  |  ${_sft_data_mode}  |  dispatcher ${DISPATCHER}
 Val         sample_ratio ${VAL_SAMPLE_RATIO} (recipe validation_dataset.sample_ratio)
...
_sft_print_submit_log_header() {
  local rel_logs rel_output rel_ckpt
  local _tp="${AUTOMODEL_NEMOTRON_OMNI_PARALLELISM[tp]}"
  local _pp="${AUTOMODEL_NEMOTRON_OMNI_PARALLELISM[pp]}"
  local _cp="${AUTOMODEL_NEMOTRON_OMNI_PARALLELISM[cp]}"
  rel_logs="$(_sft_relpath "${LOGS_DIR}")"
  rel_output="$(_sft_relpath "${OUTPUT}")"
  rel_ckpt="$(_sft_relpath "${CHECKPOINT_DIR}")"

  echo "SFT submit log — $(date '+%Y-%m-%d %H:%M:%S %Z')"
  echo ""
  echo "[config]"
  ...
  echo "  cpu_mem_env        MALLOC_ARENA_MAX=${MALLOC_ARENA_MAX} OMP=${OMP_NUM_THREADS}"
  ...
  echo ""
  echo "[paths]"
  echo "  repo_root          $(_sft_relpath "${REPO_ROOT}")"
  echo "  logs               ${rel_logs}/"
...
# Multi-account race polls until one job is RUNNING. By default, re-exec in the
# background (nohup + line-buffered stdout) so the login shell returns immediately.
...
if [[ "${SUBMIT_DETACHED:-0}" != "1" ]]; then
  if [[ "${SUBMIT_DETACH}" == "1" || "${SUBMIT_DETACH}" == "true" ]]; then
    if [[ "${SLURM_ACCOUNT_RACE}" == "1" || "${SLURM_ACCOUNT_RACE}" == "true" ]] && ((${#slurm_accounts[@]} > 1)); then
      ...
      _sft_print_detach_notice "${_submit_pid}" "${submit_log}"
      exit 0
    fi
  fi
fi
...
inference_validation_start_sbatch_watcher automodel automodel_sft
