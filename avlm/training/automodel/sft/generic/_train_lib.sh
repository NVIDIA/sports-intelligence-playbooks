...
# Generic AutoModel SFT launcher helpers.

# shellcheck shell=bash
[[ -n "${_GENERIC_TRAIN_LIB_LOADED:-}" && $(type -t generic_train 2>/dev/null) == function ]] && return 0
_GENERIC_TRAIN_LIB_LOADED=1

_GENERIC_LIB_DIR="$(cd -- "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
_AVLM_UTILS_DIR="$(cd -- "${_GENERIC_LIB_DIR}/../../../../utils" && pwd)"

generic_load_recipe_env() {
...
generic_validate_parallelism_layout() {
...
generic_resolve_output_dirs() {
  generic_validate_parallelism_layout "${NPROC_PER_NODE}"
  ...
  export OUTPUT_BASE="${OUTPUT_BASE:-${_GENERIC_LIB_DIR}/outputs}"
...
generic_init_launch() {
  export CLUSTER_PARAMS="${CLUSTER_PARAMS:-${_GENERIC_LIB_DIR}/launch_local.yaml}"
  [[ "${CLUSTER_PARAMS}" == /* ]] || CLUSTER_PARAMS="${_GENERIC_LIB_DIR}/${CLUSTER_PARAMS#"${_GENERIC_LIB_DIR}"/}"
  export CLUSTER_PARAMS
  # shellcheck source=../../../../utils/_source_params.sh
  source "${_AVLM_UTILS_DIR}/_source_params.sh"
  training_cli_commit_env_overrides "${_GENERIC_LIB_DIR}" "${GENERIC_CLI_OVERRIDE_KEYS[@]}"
  source_cluster_params "${_GENERIC_LIB_DIR}"
...
generic_train() {
  ...
  _TRAIN_SCRIPT_DIR="$(cd -- "${_GENERIC_LIB_DIR}/../slurm" && pwd)"
  # shellcheck source=../../../../utils/_train_env.sh
  source "${_AVLM_UTILS_DIR}/_train_env.sh"

  generic_load_recipe_env "${CONFIG_YAML}"
