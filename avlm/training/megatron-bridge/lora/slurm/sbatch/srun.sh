#!/usr/bin/env bash
# Run Megatron-Bridge LoRA for a submitted Slurm batch job.
...
_LORA_DIR_REL="avlm/training/megatron-bridge/lora/slurm"
_LORA_DIR="${REPO_ROOT}/${_LORA_DIR_REL}"
_MB_ROOT="${REPO_ROOT}/avlm/training/megatron-bridge"
_TRAIN_LIB="${_LORA_DIR}/_train_lib.sh"
...
srun --nodes="${SLURM_NNODES}" \
  ...
  bash -c "
    ...
    source '${_PREP}'
    mb_prepare_bridge_env '${_LORA_DIR}'
    # shellcheck source=../_train_lib.sh
    source '${_TRAIN_LIB}'
    mb_lora_init_launch batch
    mb_lora_train
  " &> "${log_file}"
