...
# Interactive Slurm session for Megatron-Bridge HF ↔ Megatron conversion.
#
...
_MODE_DIR="${SCRIPT_DIR}"
_MB_ROOT="$(cd "${_MODE_DIR}/.." && pwd)"
...
export JOB_NAME="${JOB_NAME:-mb_convert_interactive}"
...
mb_log_section "Slurm interactive session (HF ↔ Megatron conversion)"
...
srun \
  ...
  bash -c "
    ...
    mb_prepare_bridge_env \"${_MODE_DIR}\"
